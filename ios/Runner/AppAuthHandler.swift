...
class AppAuthHandler {
    ...
    func doAuthWithAutoCodeExchange(configuration: OIDServiceConfiguration, scopes: String,  clientID: String, clientSecret: String?) {
        ...
        guard let controller = registrar.viewController else {
            self.invokeErrorHandler(code: self.RUNTIME_ERROR, errorMessage: "Unable to present authorization flow", details: "Flutter view controller not available")
            return
        }

        appDelegate.currentAuthorizationFlow = OIDAuthState.authState(byPresenting: request, presenting: controller) { authState, error in
